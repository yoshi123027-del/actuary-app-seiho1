...
def render_problem_area(filtered: pd.DataFrame, menu: str, has_weekday_group: bool):
    ...
    if st.session_state.get("selected_label") not in valid_labels:
        st.session_state["selected_label"] = current_label
    elif st.session_state["selected_label"] != current_label:
        st.session_state["selected_label"] = current_label

    select_col1, select_col2 = st.columns([5, 1])
    with select_col1:
        selected_label = st.selectbox("問題選択", valid_labels, key="selected_label")
    with select_col2:
        st.write("")
        if st.button("表示", use_container_width=True):
            ...
            if new_id != st.session_state["current_id"]:
                ...
                st.session_state["selected_label"] = label_by_id[new_id]
                reset_answer_visibility()
                st.rerun()
    ...
    with nav1:
        if st.button("← 前へ", use_container_width=True):
            if current_index_for_nav > 0:
                new_id = valid_ids[current_index_for_nav - 1]
                st.session_state["current_id"] = new_id
                st.session_state["selected_label"] = label_by_id[new_id]
                reset_answer_visibility()
                st.rerun()
...
if menu == "ホーム":
    ...
    review_candidates = candidate_df[candidate_df["id"].map(lambda x: get_rating(x) == "後で復習")]
    weak_candidates = candidate_df[candidate_df["id"].map(lambda x: get_rating(x) == "わからない")]
    ...
    if st.button("今日の1問を開く", type="primary", use_container_width=True):
        target_id = str(reco["id"])
        st.session_state["menu"] = "今日の課題"
        st.session_state["current_id"] = target_id
        label_parts = [str(reco["id"]), f"第{reco['章']}章", str(reco["問題種別"])]
        if str(reco.get("問題番号", "")).strip():
            label_parts.append(str(reco["問題番号"]))
        if str(reco.get("年度", "")).strip():
            label_parts.append(f"{reco['年度']}年")
        st.session_state["selected_label"] = " | ".join(label_parts)
        reset_answer_visibility()
        st.rerun()
