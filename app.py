...
def update_flag(user_name, question_id, status):
    ...
    mask = (flags["user_name"] == user_name) & (flags["id"] == str(question_id))
    if mask.any():
...
if d >= 0:
    ...
    st.warning(f"試験日は {EXAM_DATE.strftime('%Y-%m-%d')} でした。")
...
user_name = st.sidebar.text_input("ユーザー名", value="guest").strip()
if not user_name:
    user_name = "guest"
...
menu = st.sidebar.radio(
    "メニュー",
    ["今日の課題", "章ごとに学ぶ", "問題検索", "苦手だけ解く", "教科書で学ぶ"]
)
...
if menu == "教科書で学ぶ":
    ...
    st.markdown("---")
    st.caption("要約とリンクは app.py 内の TEXTBOOK_LINKS で管理します。")
    st.stop()
...
def build_label(row):
    ...
    if str(row["学習状況"]).strip():
        parts.append(f"【{row['学習状況']}】")
    return " | ".join(parts)
...
with flag_col1:
    selected_status = st.radio(
        "この問題の状態",
        ["未解答", "苦手", "マスター"],
        index=["未解答", "苦手", "マスター"].index(current_status),
        horizontal=True
    )
...
st.caption("注意: study_flags.csv / study_time_log.csv 方式はローカル運用や小規模共有向けです。公開サイトで安定してユーザーごとに保存したい場合は、Supabase など外部DB化がおすすめです。")
