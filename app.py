...
st.caption(f"現在時刻: {now_tokyo.strftime('%Y-%m-%d %H:%M')} JST")

menu = st.sidebar.radio("学習モード", ["今日の課題", "章ごとに学ぶ", "問題検索", "教科書で学ぶ"])

if menu == "教科書で学ぶ":
    st.sidebar.markdown("### 絞り込み")
    ...
    with top_c1:
        st.markdown(f'<div class="metric-card"><div class="section-label">試験まで</div><div style="font-size:1.6rem;font-weight:700;">あと {max(days_left,0)}日</div></div>', unsafe_allow_html=True)
    with top_c2:
        st.markdown(f'<div class="metric-card"><div class="section-label">学習モード</div><div style="font-size:1.1rem;font-weight:700;">教科書で学ぶ</div></div>', unsafe_allow_html=True)
    with top_c3:
        st.markdown(f'<div class="metric-card"><div class="section-label">表示中</div><div style="font-size:1.1rem;font-weight:700;">第{selected_chapter}章</div></div>', unsafe_allow_html=True)

    st.markdown('<div class="question-card">', unsafe_allow_html=True)
    st.subheader(f"第{selected_chapter}章 教科書で学ぶ")
    st.markdown('<div class="section-label">簡易まとめ</div>', unsafe_allow_html=True)
    st.write(content["summary"])
    st.markdown('<div class="section-label" style="margin-top:14px;">教科書リンク</div>', unsafe_allow_html=True)
    if content["download_url"]:
        st.link_button("アクチュアリー会の教科書ダウンロードページへ", content["download_url"], use_container_width=True)
    else:
...
with top_c1:
    st.markdown(
        f'<div class="metric-card"><div class="section-label">試験まで</div><div style="font-size:1.6rem;font-weight:700;">あと {max(days_left,0)}日</div></div>',
        unsafe_allow_html=True,
    )
with top_c2:
    if menu == "今日の課題":
        st.markdown(
            f'<div class="metric-card"><div class="section-label">今日の残り課題</div><div style="font-size:1.6rem;font-weight:700;">{today_remaining_count}問</div><div class="small-muted">{today_jp} / グループ {today_group}</div></div>',
            unsafe_allow_html=True,
        )
    else:
        st.markdown(
            f'<div class="metric-card"><div class="section-label">表示中の問題数</div><div style="font-size:1.6rem;font-weight:700;">{len(filtered)}問</div><div class="small-muted">{menu}</div></div>',
            unsafe_allow_html=True,
        )
with top_c3:
    st.markdown(
        f'<div class="metric-card"><div class="section-label">今日の勉強時間</div><div style="font-size:1.6rem;font-weight:700;">{hours:02d}:{minutes:02d}:{seconds:02d}</div></div>',
        unsafe_allow_html=True,
    )
...
if st.session_state["timer_running"]:
    ...
    st.caption("学習開始ボタンを押すと、その日の勉強時間を計測します。")
...
def build_label(row):
    parts = [f"問{row['id']}", f"第{row['章']}章", str(row["問題種別"])]
...
with select_col:
    ...
    if st.button("この問題を開く", use_container_width=True):
...
st.markdown('<div class="question-card">', unsafe_allow_html=True)
st.subheader(title)
st.markdown('<div class="section-label">問題</div>', unsafe_allow_html=True)
st.write(row["問題文"])

show_answer = st.checkbox("解答を表示", key="show_answer")
if show_answer:
    st.markdown('<div class="answer-card">', unsafe_allow_html=True)
    st.markdown('<div class="section-label">解答</div>', unsafe_allow_html=True)
    st.write(row["解答"])
    if "解説" in row and str(row["解説"]).strip():
        st.markdown('<div class="section-label" style="margin-top:12px;">解説</div>', unsafe_allow_html=True)
        st.write(row["解説"])
    st.markdown('</div>', unsafe_allow_html=True)
st.markdown('</div>', unsafe_allow_html=True)
